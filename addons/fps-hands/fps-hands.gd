...
signal shot_fired(position: Vector3)
...
func add_recoil():
	if recoil and weapon.get_meta("recoil_x",0)+weapon.get_meta("recoil_y",0)>0:
		...
		if node_recoil_x and node_recoil_y:
...
func _on_animation_tree_animation_started(anim_name: StringName) -> void:
	...
	if anim_name == "fire":
		firing.emit()
		emit_signal("shot_fired", global_position)
		if max_magazine != 0:
			magazine -= 1
			update_inventory()
