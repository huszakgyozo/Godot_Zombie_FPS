...
func on_player_gave_damage(obj: Node3D, damage: float, point: Vector3):
	if obj == self:
...
func _physics_process(delta: float) -> void:
	if !die:
		...
		if sqr_distance_to_player <= aggro_distance * aggro_distance:
			...
			if sqr_distance_to_player < 1.5 * 1.5:
				if can_attack:
...
func _idle_patrol(delta: float) -> void:
	...
	var patrol_target = idle_center + Vector3(x, 0, z)
